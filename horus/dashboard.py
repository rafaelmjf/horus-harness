...
def _page(title: str, body: str, active: str = "projects", wide: bool = False, live: int = 0) -> str:
    ...
    return (
        ...
        "<input type='checkbox' id='skin' onclick=\"document.body.classList.toggle('skin-light',this.checked)\">"
...
def _features_summary_html(p: dict[str, Any]) -> str:
    fc = p["feature_counts"]
    total = sum(fc.values())
    if not total:
        return ""
...
def _project_column(p: dict[str, Any], i: int, aliases: list[dict[str, Any]] | None = None) -> str:
    ...
    progress = _road_progress_html(p, href=f"/project?i={i}#roadmap")
    return (
        "<article class='pcard'>"
        ...
        f"{progress}{_features_summary_html(p)}{recap}"
        "<div class='pc-foot'>"
