...
/* Phones: the fixed-height top-in row (brand + nav.tabs + theme toggle) has no
   slack, so the toggle gets pushed off-screen past the viewport edge — the one
   bug that caused horizontal overflow on every page. Reflow to two rows
   (brand+toggle, then a scrollable nav strip) instead of shrinking everything
   to illegibility. */
@media(max-width:640px){
  .wrap{padding:0 16px}
  .top-in{flex-wrap:wrap;height:auto;padding:10px 0;row-gap:8px}
  .wordmark small{display:none}
  nav.tabs{order:3;width:100%;margin-left:0;overflow-x:auto;-webkit-overflow-scrolling:touch}
...
details.launch,details.disc{border:1px solid var(--border);border-radius:var(--r-md);background:var(--panel-2);overflow:hidden;margin-top:11px}details.launch>summary,details.disc>summary{list-style:none;cursor:pointer;padding:11px 14px;font-size:13px;font-weight:500;display:flex;align-items:center;gap:9px;color:var(--ink-2)}details.launch>summary::-webkit-details-marker,details.disc>summary::-webkit-details-marker{display:none}details.launch[open]>summary,details.disc[open]>summary{border-bottom:1px solid var(--hairline);color:var(--ink)}.disc-body,.launch-body{padding:14px}.lform{display:grid;gap:11px}.frow{display:flex;gap:10px;flex-wrap:wrap}.field{flex:1 1 150px;min-width:0}.field label{display:block;font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:var(--ink-3);margin-bottom:5px;font-weight:600}select,.field input[type=text],.field input:not([type]){width:100%;font:inherit;font-size:13px;background:var(--panel);color:var(--ink);border:1px solid var(--border);border-radius:var(--r-sm);padding:8px 10px}.intent-row{display:flex;gap:10px;flex-wrap:wrap}.intent-row .btn{flex:1}
...
/* Phones: every auto-fill grid below has a fixed minmax() floor (300-372px)
   sized for desktop cards. auto-fill refuses to shrink a track below that
   floor, so on a sub-~430px viewport the single forced column runs wider than
   the screen and overflows horizontally. Drop the floor to one flexible
   column instead. */
@media(max-width:640px){
  .grid,.remote-grid,.repogrid,.sessions-grid,.settings-form{grid-template-columns:minmax(0,1fr)}
}
"""
...
def _project_column_safe(p: dict[str, Any], i: int, aliases: list[dict[str, Any]] | None = None) -> str:
    """`_project_column`, but a single project raising (e.g. its registered path was
    deleted or lost its ``.horus/`` dir after registration) must not 500 the whole
    projects section — every other project still has to render. Renders a compact
    error card in its place instead."""
    try:
        return _project_column(p, i, aliases)
    except Exception as exc:
        name = html.escape(p.get("name") or p.get("path") or f"project {i}")
        reason = html.escape(str(exc) or exc.__class__.__name__)
        return (
            "<article class='pcard pcard-error'>"
            f"<div class='pc-head'><div class='pc-title'><h3>{name}</h3></div></div>"
            f"<p class='muted'>&#9888; failed to load: {reason}</p>"
            "</article>"
        )
...
def _projects_section_html(projects: list[dict[str, Any]]) -> str:
    ...
    cards = "".join(_project_column_safe(p, i, launch_aliases) for i, p in enumerate(projects))
    if not cards:
        cards = (
...
def _accounts_strip(accounts: list[dict[str, Any]]) -> str:
    ...
    add_form = _account_add_form()
    refresh_btn = (
        "<button type='button' class='icon-btn usage-refresh' style='float:right' "
