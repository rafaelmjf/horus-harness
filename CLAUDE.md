...
> card-backed `.horus/backlog/`; retired v2 lanes are preserved in
> `.horus/archive/`. Local `sessions/` notes are optional recovery buffers, not
> required closure output. The managed block below is PRD-native (its version is in the
> block's own `horus-block-version` marker, not restated here); do not
