...
next_action: "Execute the 5-phase plan in execution.md: (1) async-load the dashboard's token-overhead + context-cache panels (the ~6.5s open is those, not markdown); (2) define the lane discipline (decisions=short topic-grouped current bullets, roadmap=top/open items, history=detail and not dashboard-loaded); (3) bake it into the horus-consolidate skill + templates + managed CLAUDE/AGENTS block, mirrored to .claude/.agents (Claude+Codex); (4) reflow this repo's decisions/roadmap/history to the spec; (5) dashboard render-to-match (trim roadmap, history open-in-editor link, decisions curated + open-full). PRs: 1 & 5 on feat/dashboard-sumi-e-redesign (#51); 2-4 on a new branch."
