...
next_action: "Start the UX-hardening plan-execution batch (see the triage note in the 'UX hardening' track): scaffold a fresh execution.md (the current one documents the finished 2026-07-01 batch), design phase first for the projection-sync 'same generation' semantics, then delegate the doctor machine-level checks + bulk `upgrade-project --all` + sync-indicator implementation to standard-tier workers with the supervisor reproducing the test gate. Meanwhile the USER is running the two-machine onboard/continue legs (started 2026-07-02 with a sibling-project onboard from machine 2) — fold any findings from that test in as they arrive; also refresh sibling repos' hooks via `horus upgrade-project --apply` once they're on ≥0.0.7."
