...
next_prompt: "Resume Horus with `horus resume` for the minimum-context handoff. FIRST `git fetch --all --prune` and verify branch state from the remote. This session shipped dashboard account controls: Control -> Accounts can add isolated Claude/Codex login-dir mappings and rename generated aliases such as `acct-9a7671`; untracked GitHub cards now explain that Onboard uses this machine's `gh` login for clone/PR while Claude/Codex account choice happens when starting agent work. Next, tighten the post-Onboard handoff so a newly tracked project immediately offers a start-work CTA with an account alias choice."
