---
status: active
current_focus: "X3 away-mode kit is primary; owner away 2026-07-22 for 6 days. Items 0-3 shipped inline 2026-07-17 (envelope, attachable+worktree unattended posture, systemd-timer scheduler, escalation channel); 4-6 remain. Item 3 (unattended-escalation-channel, PR #299) resolved its two forks: the sink is a DEDICATED Telegram bot @horus_agent_rmjf_bot (direct Bot API, needs no Hermes — that is how 'Hermes optional' is met) rather than a shared DM/topic, and default-escalation events are delivery-failed + usage-band + supervise-gate (success opt-in). Config is machine-local [notify] in ~/.horus/config.toml (token never git/fleet.toml); escalate() is best-effort and never fails the run; live-verified with `horus notify test` -> delivered. Dogfooding each item still finds the previous one's bugs."
next_action: "Continue the X3 away-mode kit before 2026-07-22: (4) supervise-verify-merge-close — the headless andon; a settled live-probe definition is the precondition for an unattended MERGE, and it consumes item 3's channel via `notify.escalate(SUPERVISE_GATE, ...)` on a red gate. Then (5) parallel-session-continuity-reconciliation (high; two incidents in 24h) and (6) cockpit-autonomous-dispatch-contract skill. Cut line unchanged: 0-3+5 give a trustable verify+escalate-only away loop."
next_prompt: "Fetch clean main. X3 (Autonomous dispatch facet) is primary; owner trip 2026-07-22. Items 0-3 shipped (escalation channel = machine-local [notify] sink; dedicated Telegram bot; best-effort). Next is item 4 supervise-verify-merge-close: a headless supervise loop that independently verifies (required CI on the exact SHA + freshness + one live probe, never worker self-report), then merges/closes on green or escalates via `horus notify` (SUPERVISE_GATE event) on a red gate. Branch -> PR; reproduce every gate, drive the real surface once. Work inline."
execution_recommendation: "continue-as-is — items 4-6 are design-heavy (what a live probe means before an unattended merge; what counts as a reconcilable sibling delivery) and share horus/cli.py + run_executor, so they land sequentially. Work INLINE: the 2026-07-17 dispatch-test of item 3 was cancelled (owner: that experiment won't work on this machine, retry later); the session's own evidence favours inline for this interactive, gate-heavy work."
