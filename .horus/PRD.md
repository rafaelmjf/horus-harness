---
status: active
current_focus: "2026-08-03 — one card, and the finding that rescoped it. `session-close-ux-and-truthful-end-state` asked whether to RETIRE the Sessions section or repair the states; the owner's condition was 'if nothing depends on the status column'. It does — `is_attachable` and `is_restorable` both gate on the literal `stale` — and deletion proved the MOST expensive option, not the cheapest, since the section carries the PTY tabs, `/session-dismiss` and the xterm JS. The bigger find: #489 shipped `is_deliberate_close` with ZERO production callers, so its claim that 73 rows were 'correct with no backfill' held for the predicate and for no surface — 74 of 252 still read `failed`. #498 gives it a caller on `horus sessions` only, the dashboard being unused."
next_action: "Two owner decisions, either order. (a) BUILD `account-login-verb` — the only Ready card, and the only chain where finishing one unblocks another: it lifts the live gate on `codex-isolated-config-leak`, the one real bug left. One provisioning function in `config.py` consumed by `horus account --login`, the dashboard and a TUI affordance; Ready-ATTENDED because verifying it needs a real interactive agent login, i.e. the owner at the keyboard. (b) A RELEASE has the most concrete argument, though it stays its own decision with the owner: EIGHT commits (#491-#498) sit ahead of v0.0.81, and the skill-usage hook calls `horus hook skill-invoked`, which 0.0.81 rejects (exit 2, swallowed by `|| exit 0`) — so `horus skill usage` records NOTHING until one ships."
next_prompt: "Resume horus-harness; `git fetch --all --prune` first. Main is clean; v0.0.81 is the last tag and EIGHT product commits (#491-#498) are ahead of it, so the installed CLI is BEHIND the repo — do not trust `horus --help`/`--version` for anything added since. Backlog is 3 active: read `horus backlog list`. Both open items wait on the owner: building `account-login-verb` (lifts the gate on `codex-isolated-config-leak`, the one real bug left) and cutting a release (the skill-usage hook stays inert until one ships). Before touching the cockpit, know that the owner is not using the hosted dashboard at all — the Dashboard/cockpit facet reads as zero OPEN cards while nowhere near its definition of done, because its cards are shelved, not converged."
execution_recommendation: "continue-as-is — no delegation was requested; both open items are single bounded tasks, and the release runbook is the `horus-release` skill rather than a plan."
...
- **One status list, imported everywhere (2026-08-01, #481/#485/#486/#489/#498).** `backlog.INACTIVE_STATUSES` is the one definition; three hand-copies each lied differently. The recurring shape is **a display aggregating over states it does not distinguish** — last where `failed` covered both a crash and a deliberate close. Two corollaries: **a missing word hides truth rather than losing it** (the intent sat in `termination_reason`, so reading the *pair* needed no backfill), and **a predicate with no caller fixes nothing** — #489 left `is_deliberate_close` uncalled, so assert the SURFACE, not the helper.
