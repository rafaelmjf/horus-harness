---
status: active
current_focus: "v0.0.34 released and live (PRs #134-137). Three side items out for cockpit review, none auto-merge: `horus wiki` (draft PR #138), `horus capabilities` (draft PR #139), and an overseer-dispatched `horus run --effort` passthrough (PR #140, adapter-layer, doesn't touch the seam below). Flagship next step is unchanged: the LaunchBackend seam."
next_action: "Opus inline freezes the LaunchBackend seam + LocalBackend before RemoteBackend/ContainerBackend delegation (harness P0 of the multi-machine arc). Separately, review draft PRs #138/#139/#140 — decide keep/shape/merge on each; see backlog cards."
...
**Reasoning-effort passthrough** (2026-07-10, PR #140, overseer-dispatched, unmerged): `horus run --effort {low,medium,high,xhigh,max}` reaches both real adapters via `SpawnSpec.effort` — claude forwards it verbatim (Claude Code's own CLI documents `--effort` with this exact enum, confirmed live on 2.1.206, so no no-op/warning shim was needed); codex has no dedicated flag, so it's mapped to the documented `-c model_reasoning_effort=<value>` config override (server-validates, doesn't silently drop unsupported levels).
