---
status: active
current_focus: "2026-08-07 — built bundled project skill `publish-openwiki-site` v1 on `feat/publish-openwiki-site-skill`. `horus/skills.py` is canonical and Horus generated byte-identical Claude and Codex projections. The workflow separates generation from publication, keeps each OpenWiki origin on loopback, requires Access before DNS/tunnel exposure, detects OpenWiki's silent port shift, deploys an exact pushed SHA from an isolated checkout, and defines local/remote/rollback gates. Both projections validate; affected suites pass 269."
next_action: "Review and merge the `publish-openwiki-site` PR, then exercise it on the separate `experiment/openwiki` branch to publish that wiki behind Cloudflare Access. Before a Horus release, the merged working tree can project it into a target with `uv run horus skill install --path <project> --target all`; fleet propagation through the installed CLI still needs its own release decision. Main's earlier `account-login-verb` and release choices remain open."
next_prompt: "Resume horus-harness; `git fetch --all --prune` first. The new project-scoped `publish-openwiki-site` v1 is on `feat/publish-openwiki-site-skill`, canonical in `horus/skills.py` and projected identically to `.claude/skills/` and `.agents/skills/`. It passed both skill validators and 269 affected tests. The OpenWiki content experiment remains on sibling branch `experiment/openwiki`; publication is the next real skill trial after this branch lands. The installed 0.0.81 CLI does not contain this skill until a later release."
execution_recommendation: "continue-as-is — no delegation was requested; merging and exercising this one bounded workflow is direct work."
