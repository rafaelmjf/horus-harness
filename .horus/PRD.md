---
status: active
current_focus: "2026-08-13 — The curator ledger foundation shipped across three PRs (#508 `1dff05c`, #509 `e31089e`, #510 `2f4957b`): `horus curate` deterministically captures + redacts + git-remote-attributes local Claude/Codex sessions; `--interpret` runs batched LLM curation via the authenticated native `claude -p` on a cheap tier; `--portfolio` assembles a regeneratable git-of-record (curation + skeleton only, raw stays local). Split out of the `curator-portfolio-plan` anchor this session via backlog-refine. Required pytest 3.12/3.13 + freshness green on each exact head; live probes: 82 sessions / 19 secrets redacted, one real haiku curation, 23-project portfolio with 0 leaks."
next_action: "⏳ PENDING HOSTED DEPLOY: v0.0.82 is published to PyPI but horus.rafaelfigueiredo.com still runs 0.0.81 — run `scripts/deploy-hosted.sh` ON the server (owner deferred it 2026-08-13). Curator ledger is fully built and its curation is populating the local view via `horus curate --interpret --portfolio` (cheap-tier, watermarked). Back to the owner-prioritized backlog: the `curator-portfolio-plan` anchor stays Shaping with two [session] opens — seamless trigger and proposal-into-project lifecycle — best resolved once the ledger is in daily use. `curator-account-label-doubling` is a filed low bug (view already maps labels; manifest still doubles). `codex-isolated-config-leak` Gated; `rework-direction-skills-onto-topics` Deferred."
next_prompt: "Resume from synced `main`. The curator ledger (curator-ledger-foundation) is fully shipped through #512: `horus curate [--interpret] [--portfolio]` — capture, per-session structured curation, raw-free git-of-record, and a LOCAL sumi-e view (out_dir/portfolio.html) with transcript drill-down that reproduces the 2026-08-12 design. The owner chose the local-artifact path; a backend is deferred to the anchor's phase-6 'conditional door' (only for write-back / phone-live / search-at-scale). Strategic opens still live in the `curator-portfolio-plan` anchor (Shaping): seamless trigger, proposal-into-project flow. Read the backlog before follow-up."
execution_recommendation: "continue-as-is — delivered inline across three sibling PRs; no delegation requested and none needed for the remaining backlog."
...
**The cross-surface session ledger — `horus curate`** (2026-08-13, #508 `1dff05c` + #509 + #510 + #511 + #512 `982054f`): deterministic capture of local Claude/Codex sessions (dual-schema parse, secret redaction+scan, git-remote attribution with self-flags, content-hash watermark); `--interpret` batches **per-session structured** LLM curation (`curation/<slug>.json`: context + segments + Discussed/Decided/Shipped/Open) through the authenticated native `claude -p` on a cheap tier (no inference dependency); `--portfolio` produces **two outputs** — a raw-free **git-of-record** (skeleton + curation.json + index.md, no HTML committed) and a **local sumi-e view** (`out_dir/portfolio.html`) that reproduces the 2026-08-12 rafaelfigueiredo.com design from a generated DATA/RAW payload, with redacted-transcript drill-down. The genuine template lives at `horus/assets/portfolio_view.html`. Phases 1-3 of `curator-ledger-foundation`, ported from the 2026-08-12 PoC. Two curation-robustness fixes surfaced only under the first full 23-project run (#513 stdin, #514 `5cdef37`): the bundle must ride on `claude -p` stdin (Windows caps argv) from a neutral cwd, and — because a truncated transcript ends mid-conversation — the instruction goes LAST with the transcript wrapped as delimited DATA, or the model continues the chat instead of summarizing; `_parse_model_json` extracts the first balanced object, tolerating prose/fences. **Design decision (owner, 2026-08-13):** the rich view stays a LOCAL artifact (raw drill-down); the git repo is raw-free/portable. A backend was weighed and declined for now — it only earns itself for write-back / live phone access / search-at-scale (the anchor's phase-6 "conditional door").
...
**Generated context measured against the repo-native baseline — both tools dropped** (2026-08-09; card `openwiki-graphify-value-benchmark` carries the verdict): six Opus 5 sessions, two task shapes, one pinned SHA — zero accuracy difference, zero false claims, baseline cheapest.
...
- **A derived surface can never be clearer than its source, so `## Shipped` is authored for a zero-context reader (2026-08-09).** One catalogue serves owner and outsider identically — no internal register, no separate adoption field to drift apart. Enforced where entries are written (`horus-consolidate`), never at a renderer. Richer entries cost budget, so aged ones distil out to `.horus/archive/history.md` sooner.
