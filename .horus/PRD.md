---
status: active
current_focus: "v0.0.59 SHIPPED (2026-07-17) — tagged v0.0.59, GitHub Release published, PyPI Trusted Publishing job succeeded (0.0.59 is latest), install-smoke green on all 3 OSes, hosted dashboard deployed via scripts/deploy-hosted.sh (/health==0.0.59, / still 403). Released the merged PO-lifecycle work (PRs #280–#284): the phase-aware `horus consolidate` convergence read-out (cards carry `vision_facet`/`phase`, skill v12) + the outward `market-scan` skill. The read-out is inert without a `## Vision` facet table, so existing projects still need facets authored + cards stamped — that assisted flow is the next build, horus-kickstart (phase: explore)."
next_action: "Build horus-kickstart (phase: explore) per its card — a thin orchestrator skill that SEQUENCES existing skills (market-scan → deep-research → the horus consolidate convergence read-out) and pauses for owner approval at every gate: introspect repo/PRD, market-scan (respect shipped ledger), propose a facet DIFF (add/rename/retire/promote, never wholesale replace), propose exploratory backlog cards, propose execution order. Advisory/diff-only (never auto-applies); onboarding folds in (no facet table ⇒ propose initial facets + offer to stamp cards); state+confirm a token envelope before web work; works on v3 and v2. Open design question: how much ordering/facet-diff is deterministic CLI signal vs agent judgment (lean CLI for signals, agent for judgment)."
next_prompt: "Fetch clean main. v0.0.59 is fully shipped (tagged/published/deployed). Build horus-kickstart per .horus/backlog/horus-kickstart.md: a thin orchestrator skill sequencing market-scan → deep-research → the horus consolidate convergence read-out, pausing for owner approval at each gate; proposes a Vision facet DIFF + exploratory backlog cards + execution order; advisory/diff-only, never auto-applies; folds in onboarding for projects with no facet table; confirm a token envelope up front. Branch → PR."
execution_recommendation: "continue-as-is — horus-kickstart is an exploratory (phase: explore) guided-flow design that composes skills already in the repo; single-agent inline is cheapest with no parallelism or price dividend, and the design questions want tight iteration, not a phased worker plan."
...
Twelve active cards, each stamped with a `vision_facet`: medium — fleet artifact refresh, stale-datum overlap reconciliation, remote open-model probe, project workflow overrides, scoped machine requirements, and two PO-lifecycle cards (horus-kickstart — the `phase: explore` guided re-baseline + onboarding flow; explore-converge-lifecycle — open only for the usage-ripeness flag); low/deferred — Codex usage-window semantics, completion-receipt trimmings, init-CI, heartbeat stall detection, and product-naming (rename pinned to first external distribution; horus-po/horus-continuity both rejected — want a more creative name).

## Shipped

**PO-lifecycle: vision-convergent roadmap + outward market-scan** (2026-07-16/17, PRs #280–#283): Vision widened to a repo-local product owner resolved into 7 facets each with a definition of done, plus a breathing divergence→convergence model; backlog cards carry `vision_facet`/`phase` and `horus consolidate` emits a phase-aware convergence read-out (per-facet coverage + exploratory bucket; warns off-vision/unknown-facet converge cards; horus-consolidate skill v12); the `market-scan` skill (outward twin of product-audit) composes deep-research into a dated `.horus/research/` receipt proposing Vision text + candidate cards (advisory, never auto-writes). Dogfooded on the agent-PO tooling landscape.
