---
status: active
current_focus: "RELEASE v0.0.59 IN PROGRESS (2026-07-17) — the three-file version bump (pyproject + horus/__init__.py + uv.lock, 0.0.58→0.0.59) is MERGED to main via PR #284; main is at 0.0.59. NOT yet tagged/published/deployed. This releases the merged PO-lifecycle work (PRs #280–#283): the phase-aware `horus consolidate` convergence read-out (cards carry `vision_facet`/`phase`, skill v12) and the outward `market-scan` skill (composes deep-research → dated `.horus/research/` receipt, advisory). The convergence read-out is deliberately inert without a `## Vision` facet table (non-invasive), so existing projects still need facets authored + cards stamped — carded as horus-kickstart (phase: explore). Checkpointed here because the 5h usage window was near its limit."
next_action: "FINISH releasing v0.0.59 (bump already merged, main at 0.0.59). Remaining steps in order: (1) tag `v0.0.59` on main and `gh release create v0.0.59` (publish.yml triggers PyPI trusted publishing on the published Release); (2) verify — PyPI JSON/simple-index shows 0.0.59 AND a fresh `uv tool install --force --python 3.12 horus-harness` resolves 0.0.59 (watch install-smoke.yml across the 3 OSes); (3) run `scripts/deploy-hosted.sh` (upgrades the pinned hosted install with --refresh, restarts horus-dashboard.service, verifies /health version==0.0.59 AND / still 403s). Then the next build is horus-kickstart (phase: explore) per its card."
next_prompt: "Fetch clean main (at 0.0.59). Release v0.0.59 is mid-flight: the version bump merged (#284) but it is NOT tagged/published/deployed. Finish it: (1) tag v0.0.59 + `gh release create v0.0.59` → PyPI publish; (2) verify PyPI + fresh `uv tool install --force --python 3.12` + install-smoke green on 3 OSes; (3) `scripts/deploy-hosted.sh` and confirm /health==0.0.59 + / 403. Release invariant: a green publish job alone is NOT enough — deploy-hosted.sh is the final required rung. After the release, build horus-kickstart per its card."
execution_recommendation: "continue-as-is — finishing the release is a bounded scripted checklist (tag → release → verify → deploy-hosted); no parallelism or cross-project dividend, single-agent inline is cheapest."
