---
status: active
current_focus: "Codex stale usage-cache preflight bug fixed in PR #145: expired 5h/weekly reset windows no longer refuse on cached percentages. Omnigent LaunchBackend fit spike remains complete; draft PRs #138/#139 remain out for review; flagship next step remains the LaunchBackend seam."
...
**Expired usage-window preflight guard** (2026-07-10, PR #145): `horus run` now treats any 5h/weekly usage window whose reset time is at or before now as expired per-window, tries one best-effort refresh, and otherwise routes it through the existing unknown-capacity path; future-reset near-limit windows still refuse.
