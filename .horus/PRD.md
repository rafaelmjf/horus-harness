---
status: active
current_focus: "2026-08-01 — v0.0.81 shipped, released, published and deployed, carrying the fourth and last instance of the day's defect class: `registry.STOPPED` means the owner closed the session, so 73 of 83 `failed` rows stop rendering red, with no backfill (the pair status+`termination_reason` already held the truth). The release itself was the day's second experiment: a dispatched gpt-5.6-luna@max worker ran the whole 10-step chain — bump PR, CI, merge, tag, GitHub release, PyPI, `deploy-hosted.sh` — in 362s, and did NOT touch `.horus/`, which both earlier drill legs had done unprompted. Backlog stays 7 active."
next_action: "Repair the unreachable gate carried across three closes now: `codex-isolated-config-leak` is active and Gated on `account-login-verb`, which is `status: shelved`, so nothing can ever lift it. Pick one — unshelve the blocker (its own deferral trigger, an interactive login, expired 2026-07-29), re-point the gate, or shelve the dependent. It is the only active card in that state; the other cards with shelved blockers are themselves shelved. After that the most concrete Shaping card is `stale-worktree-accumulation`: the measurement is done (5 of 6 non-main worktrees provably dead), only the decision of where the routine lives is open."
next_prompt: "Resume horus-harness; `git fetch --all --prune` first. Main is clean, v0.0.81 is released, published and deployed — installed CLI, hosted `/health` and repo all read 0.0.81. Backlog is 7 active: read `horus backlog list` (and `--shelved` for the 64 boxed ones), never the directory listing. `session-close-ux-and-truthful-end-state` stays OPEN on purpose — #489 shipped the repair for the one path that had a signal to discard, while the owner's standing review says the preferred end state may be retiring the Sessions status column entirely; read that card's Reviews before touching it. The immediate item is the `codex-isolated-config-leak` gate, which points at a shelved card."
execution_recommendation: "continue-as-is — the next item is a one-card backlog decision, and the two experiments this repo owed the delegation facet (away-mode drill, worker-run release) are both done."
...
- **One status list, imported everywhere (2026-08-01, #481/#485/#486/#489).** `backlog.INACTIVE_STATUSES` is the one definition; three hand-copies existed and each lied differently. The recurring shape is **a display aggregating over states it does not distinguish** — four times in one release, last in the session registry, where `failed` covered both a crash and a deliberate close. Corollary: **a vocabulary missing a word hides the truth rather than losing it** — the intent already sat in `termination_reason`, so reading the *pair* fixed 73 rows with no backfill. (`archive/history.md`.)
...
- **Name the off-limits surface in the brief; an unstated expectation is not a worker defect (2026-08-01).** The same model and effort that overwrote `.horus/` on both drill legs left it untouched across a ten-step release once the brief named it a hard constraint — so "the worker is unreliable" was the wrong reading of the drill. This does NOT relax the rule above: one clean run shows this model can follow this runbook once, not that unattended release is safe, and its irreversible steps were delegated on the owner's explicit instruction. (`archive/history.md`.)
...
- **Anything PROJECTED into every project is a product surface FOR those projects — bundled skills AND the managed block (2026-07-30/31).** Write and audit it from a consumer's seat; block rules must be general, and this repo's incidents belong in `archive/history.md`. Two corollaries: **a stale skill is a stale program** (*behaviour is correct because it lives in code* fails where the text IS the behaviour), and **a bundled-skill fix reaches the fleet only through a RELEASE** — `upgrade-project --apply` before a release cut installs the pre-fix version. (#462/#463/#466/#471; card `skill-drift-surfacing-and-refresh`.)
...
- **Bundled skill edits bump the skill version, always.** The version-aware install skips same-version content, so an unbumped text change leaves committed projections silently stale (observed: #247 fleet-curation drift, caught 2026-07-16). Resync with `skill install --force`; never hand-edit the projected `SKILL.md` copies. `Skill(name, N, text)` and the `horus-skill-version: N` marker INSIDE that text are two copies of one number — when they disagree every `upgrade-project` run reports "updated" forever while doctor never goes green; a test pins their agreement (2026-07-31).
...
- **Worker accounting captures one end reading, and never estimates.** Show a delta only for fresh same-window isolated readings without tracked overlap; otherwise label it unknown or confounded. Never estimate task usage, auto-route from cost, poll, or add a model call. **Runtime is a weak proxy for task size** (2026-08-01: 362s for a ten-step release vs 481s for a two-line edit, same model and effort). Workflow tests require a real distinct worker; Codex auto-edit workers get a read-only `.git` and no socket bind — the supervisor owns commit, push and every runtime gate.
