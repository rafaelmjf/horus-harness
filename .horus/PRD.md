...
**Mobile-web-app bundle** (2026-07-11, PRs #149/#151/#153): `usage-reset-inference` (dashboard degrades a cached window past `resets_at` to "reset — capacity available" instead of a stale/fabricated percent, plus a zero-network `GET /accounts-refresh` cache reread), `pwa-installable` (`/manifest.json` + `/sw.js`, cache-first ONLY for a fixed static app-shell whitelist — every HTML/data route always hits the network behind Cloudflare Access), `responsive-mobile-pass` (five phone-width CSS overflow fixes in `_STYLE`, desktop unchanged) — full detail in git history; `mobile-terminal-interaction-regression` remains open (Backlog).
