---
status: active
current_focus: "PR #167 (price-for-capability model-roster priors) MERGED. This session implemented the `model-name-normalization-and-datum-migration` backlog card (owner-locked: real rename, not an alias/mirror) — measured datums and owner priors/pricing were keying on different names (`sonnet` vs `sonnet-5`) so they never joined into one roll-up row. `horus run` now canonicalizes the captured model via `datums.canonical_model_name`, preferring the adapter's resolved concrete model over a small owner-maintained fallback map; `horus datum migrate-names` does a one-time idempotent bare->canonical rename of datums.json (byte-stable re-run, verified manually); `capabilities --models`/`--matrix` render an aligned table (model/tier/datums/last/price/capability/researched) instead of the old vertical block, with strength/caution/guard moved to a Notes section. 1208 tests green. PR #168 (branch feat/model-name-normalization) open, awaiting overseer review — not merged per task boundary."
next_action: "Confirm PR #168 required CI is green on its head SHA, then merge (or await overseer merge per repo policy). Once merged, pick an open thread: mobile-terminal-interaction-regression (.horus/backlog/, high), the parallel-safety multi-worker contention test (PR #148), accounts-refresh-button-invisible (medium, one-line CSS), or `model-ranking-synthesis` (.horus/backlog/, the deliberately-separate ranking card this PR did NOT build). [tier: Sonnet for the bug cards.]"
next_prompt: "Resume Horus. FIRST git fetch --all --prune and read .horus/PRD.md plus the newest .horus/sessions/ note. Check PR #168 (model-name-normalization) merge status; if merged, verify and archive its session note. Otherwise pick an open thread: mobile-terminal-interaction-regression (.horus/backlog/, high), parallel-safety multi-worker contention test (PR #148), or accounts-refresh-button-invisible (one-line CSS). [tier: Sonnet]"
