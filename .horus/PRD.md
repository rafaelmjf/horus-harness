---
status: active
current_focus: "pathfinder skill SHIPPED + first dogfood done (2026-07-17, PRs #285 build, #286 rename/improve; both merge SHAs 653f982 + 026ffbd CI-green). The `phase: explore` guided one-pass divergence→convergence re-baseline skill — a thin orchestrator that SEQUENCES market-scan → deep-research → the `horus consolidate` read-out with an owner gate at every step; proposes a Vision facet DIFF + exploratory cards + execution order; advisory/diff-only; onboarding folds in. Then DOGFOODED it on this very repo, which is what earned #286: the run broke on real SKILL gaps (not product) — (1) it read as onboarding-only, (2) it silently ASSUMED an outward adoption frame when the owner's intent was deepen-own-use (build-vs-adopt), (3) research ran before pinning shipped+vision+audience. #286 fixed all three: renamed `horus-kickstart`→`pathfinder` (unprefixed, age-agnostic; kickstart stays a trigger word), added a Step 0 intent gate (deepen-own-use|broaden-adoption|both) that steers the research frame + verdict, grounded Step 1 with a pinned brief passed to the scan as a hard constraint, and market-scan v2 gained the build-vs-adopt verdict. Product finding from the dogfood: build verdict is settled — cross-agent (2 Claude + 1 Codex, more coming) × cross-project fleet × PO-lifecycle is uncontested; only the atomic memory primitive is commoditized (adopt underneath, don't compete). Receipt: .horus/research/2026-07-17-repo-local-po-rebaseline.md."
next_action: "Dogfood pathfinder AGAIN on this project — a second guided re-baseline pass now that the Step 0 intent gate + grounded brief + build-vs-adopt frame are in. This time declare intent=deepen-own-use up front and carry it through: introspect → pinned shipped+vision+audience brief → (envelope-gated) market-scan read as build-vs-adopt → propose a Vision facet DIFF + exploratory cards + order. Goal is twofold: (a) actually produce a facet diff/backlog this run (the first run stopped at the skill-gap finding before Step 3), and (b) verify the v2 flow feels right — capture any remaining rough edges for a skill-audit. Candidate directions already surfaced by the first scan: rescope Continuity-core DoD to lead with agent-neutrality; [explore] native-memory interop (adopt CLAUDE.md/memory-bank/.clinerules INTO .horus/); [explore] sit-above-spec-pipelines (compose Spec-Kit/Task-Master)."
next_prompt: "Fetch clean main. pathfinder is shipped + improved (PRs #285/#286). Invoke the `pathfinder` skill on THIS repo for a second dogfood: declare intent=deepen-own-use, run Step 0→Step 6, and this time carry through to an actual proposed Vision facet DIFF + exploratory cards + execution order (the first run stopped early at a skill-gap finding). Reference the prior receipt .horus/research/2026-07-17-repo-local-po-rebaseline.md and the candidate directions in next_action. Capture any remaining rough edges as skill-audit fodder."
execution_recommendation: "continue-as-is — a single exploratory dogfood pass (pathfinder on one repo); small, iterative, owner-in-the-loop at every gate, no parallelism or lower-tier dividend that would beat the supervisor tax."
...
Twelve active cards, each stamped with a `vision_facet`: medium — fleet artifact refresh, stale-datum overlap reconciliation, remote open-model probe, project workflow overrides, scoped machine requirements, and the lone remaining PO-lifecycle card explore-converge-lifecycle (open only for the usage-ripeness flag; pathfinder skill shipped in #285/#286); low/deferred — account-settings-sync (sync one settings block across isolated account dirs), Codex usage-window semantics, completion-receipt trimmings, init-CI, heartbeat stall detection, and product-naming (rename pinned to first external distribution; horus-po/horus-continuity both rejected — want a more creative name).

## Shipped

**PO-lifecycle: vision-convergent roadmap + market-scan + `pathfinder` re-baseline skill** (2026-07-16/17, PRs #280–#283/#285/#286): Vision widened to a repo-local product owner resolved into 7 facets each with a definition of done, plus a breathing divergence→convergence model; backlog cards carry `vision_facet`/`phase` and `horus consolidate` emits a phase-aware convergence read-out (per-facet coverage + exploratory bucket; warns off-vision/unknown-facet converge cards; horus-consolidate skill v12); the `market-scan` skill (outward twin of product-audit, now v2 with a build-vs-adopt verdict for personal-tooling intent) composes deep-research into a dated `.horus/research/` receipt proposing Vision text + candidate cards (advisory, never auto-writes). `pathfinder` (skill v1, PR #285 built as `horus-kickstart`, PR #286 renamed + improved) is the `phase: explore` thin orchestrator sequencing market-scan → deep-research → the consolidate read-out with an owner gate at every step, proposing a Vision facet DIFF + exploratory cards + order (advisory/diff-only; onboarding folds in) — no new CLI, since the facet→card mapping is the existing read-out. #286 came from dogfooding it on this repo, which surfaced three SKILL gaps: onboarding-only name → renamed to age-agnostic `pathfinder`; silent frame assumption → Step 0 intent gate (deepen-own-use|broaden-adoption|both); ungrounded research → Step 1 pins a shipped+vision+audience brief passed to the scan as a hard constraint.
...
**Terminal cockpit + mobile/multi-viewer stack** (2026-07-12/13, PRs #171–#215/#178–#191, v0.0.36–0.0.52): responsive phone/desktop TUI (KPIs, scrolling, unified Resume/Fresh, card resume, live-session controls, managed-tmux attach across viewers, scoped mouse, `d`-Defaults launch posture, positive-confirmation orphan-reaper); phone-width spawn + retained exits + SSE reset + verified native iOS Tailscale SSH→tmux; `dashboard --reload`/`app` child respawn; Codex account-wide usage scope; consolidated-to gitignore marker; Windows `fcntl` lazy-import (three-OS smoke green).
