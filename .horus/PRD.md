---
status: active
current_focus: "horus-hub Phases 0 and 1a shipped 2026-07-04 (hub PRs #1–#2): auth skeleton via codex worker, read-only project inventory via a claude/work worker in a worktree (user-routed) — both accepted on reproduced gates + live probes, CI green on exact shas and main pushes. Hub Phase 1 remainder: phase1-access-jwt, blocked on the stdlib-vs-dependency decision (in the hub PRD). Hub branch protection still blocked (private repo, free plan)."
next_action: "In ~/projects/horus-hub: make the phase1-access-jwt call (pure-stdlib RS256 verify vs first justified dependency) with Rafa, then spawn that worker per the hub's .horus/execution.md. In this repo the backlog is the menu (catalog niceties / Windows ops / macOS pass are the top candidates)."
next_prompt: "Resume Horus. FIRST git fetch --all --prune and verify the branch against origin. Read .horus/PRD.md. Hub work continues in ~/projects/horus-hub — its PRD + .horus/execution.md carry phase1-access-jwt (needs the stdlib-vs-dependency decision before spawning). This repo's own backlog is otherwise the menu."
