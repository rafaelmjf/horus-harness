---
status: active
current_focus: "PO-lifecycle capabilities shipped (2026-07-16/17, PRs #280–#283): vision-convergent roadmap (7 facets + DoD + breathing divergence→convergence model + phase-aware `horus consolidate` read-out, skill v12) and the outward `market-scan` skill (twin of product-audit, composes deep-research → dated `.horus/research/` receipt, advisory). Both merged inline, cards archived. The convergence read-out is deliberately inert on projects without a `## Vision` facet table, so a release ships it non-invasively but existing projects need facets authored + cards stamped — that assisted onboarding is now carded as horus-kickstart (phase: explore), which also delivers the owner's guided re-baseline use case. v0.0.58 still the released CLI; the merged read-out/market-scan are NOT yet in the installed/hosted CLI."
next_action: "Owner's call between two next steps. (a) RELEASE — bump pyproject.toml + horus/__init__.py + uv.lock, publish, prove PyPI + 3-OS install, then scripts/deploy-hosted.sh — to land the convergence read-out + market-scan in the installed/hosted CLI. Recommended first: three merged capabilities are inert in the installed CLI until released, and horus-kickstart needs the read-out present in a TARGET project's CLI to work on other repos. (b) BUILD horus-kickstart (phase: explore): the guided introspect→market-scan→facet-diff→cards→order→converge re-baseline flow; advisory/diff-only, facet-diff-not-replace (guardrails on the card)."
next_prompt: "Fetch clean main, read PRD.md (Vision = repo-local product owner, 7 facets) and .horus/backlog/. PO-lifecycle discovery+convergence shipped (#280–#283); market-scan + the consolidate read-out are merged but NOT yet released. Pick: (a) cut a release (three-file bump → publish → 3-OS smoke → scripts/deploy-hosted.sh) so the new capabilities reach the installed/hosted CLI; or (b) build horus-kickstart (phase: explore) per its card — the guided re-baseline + onboarding flow composing market-scan + the read-out. Recommend (a) first."
execution_recommendation: "continue-as-is — a release is a bounded scripted checklist and horus-kickstart is a single-repo skill-authoring bet; neither has a cross-project or parallelism dividend, single-agent inline is cheapest."
