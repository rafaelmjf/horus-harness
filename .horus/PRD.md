...
next_action: "Owner's call among four, no dependency between them. (0) Shape `remote-sourced-project-view` — the most concrete new work: show every project on remote with its feature branches and backlog cards, without cloning. `github_catalog.discover` already reads remote `.horus/` continuity without cloning and `dashboard.gather_remote_projects` consumes it, so the gap is exact: `RemoteProject` carries `default_branch` only and nothing reads remote `.horus/backlog/`. Its stated first step is rewording the facet's definition of done, which still promises web/phone launching the owner no longer wants. (a) Run `backstage-catalog-prior-art-scan` — a shallow, bounded prior-art pass before building any catalogue site; it is Ready/attended and the only new card. (b) Act on the AUTHORING RULE below: `## Shipped` entries must stand alone for a zero-context reader, which is the real blocker on a capability catalogue and improves what the owner already reads even if no site ships. (c) Fix the two `horus capabilities` defects — entries clipped at a line boundary where a PRD hard-wraps (all 11 `horus-agent`, 5 of 6 `horus-hub`), and fleet mode omitting `vision` (empty 7 of 7) plus `generated_at`/`horus_version` that project mode populates. A release also has a standing argument — 21 commits now sit ahead of v0.0.81 — but it is its own decision with the owner."
next_prompt: "Resume horus-harness; `git fetch --all --prune` first. The OpenWiki/Graphify question is CLOSED — read the `## Verdict` section of `.horus/backlog/openwiki-graphify-value-benchmark.md` before reopening any part of it; both tools dropped for agent use on six runs, and the owner closed the human question separately. `experiment/graphify` and `experiment/openwiki` are deleted; `bench/baseline|graphify|openwiki` remain as the reproducible evidence at `e600407` (graphify's artifact rebuilds free in 20s, openwiki's costs a ~19-min run on the owner's ChatGPT Plus quota). The live published site `horus-wiki.rafaelfigueiredo.com` is STILL RUNNING (`openwiki-horus-harness.service`, loopback 8790) serving content from the now-deleted branch — taking it down is an owner call that was not taken. Direction context, all brainstorm and nothing decided: the owner wants the dashboard re-scoped from actuation to PUBLICATION of remote state, because the TUI only sees locally-cloned projects; the data paths already exist (`github_catalog.discover` reads remote `current_focus`/`next_action` without cloning and `dashboard.gather_remote_projects` already consumes it, treating remote as a supplement to local rather than the primary view; `horus backlog --tree --json` is already a board model; `horus capabilities` has fleet and per-project modes). Two boundaries to respect rather than drift past: the Vision explicitly declined boards, and the facet's definition of done still says 'launches/resumes any project from web or phone', which the owner no longer wants — rewording it is the cheap first move. `graphify-out/` (9.7 MB) is still untracked in the main working tree — that particular generation's branch (`experiment/graphify`) is deleted, though a fresh one built at `e600407` is committed on `bench/graphify`, so nothing unique is at risk. It is regenerable in 20s and belongs to a dropped tool, so removing or ignoring it is now an easy owner call rather than a real decision; it is the only thing keeping `close --check` from green."
...
**Generated context measured against the repo-native baseline — both tools dropped** (2026-08-09, no PR; card `openwiki-graphify-value-benchmark` carries the verdict): six Opus 5 sessions over two pilot tasks from one pinned SHA showed zero accuracy difference and zero false claims in every cell, with baseline cheapest both times; Graphify's as-shipped install additionally injects "MANDATORY … you MUST run graphify before reading source files" on every read, so its arm tested graph-plus-coercion and still tied.

**`horus close --check` no longer claims canonical continuity while unmerged** (2026-08-09, #500, `538ab39`): `continuity_off_default()` keys off the canonical checkpoint rather than HEAD, the unmerged-branch line names the branch carrying continuity `origin/<default>` lacks, and the push finding names the upstream it actually verified.

**All accounts' usage refreshes live from the TUI** (2026-08-09, #504, `e600407`): the `U` key; pinned as the benchmark's base SHA because it touches `terminal_tui.py`.
...
- **A derived surface can never be clearer than its source, so `## Shipped` is authored for a zero-context reader (2026-08-09).** One catalogue serves owner and outsider identically; there is no internal register and no separate adoption field to drift apart. Each entry must stand alone — what the capability is, who it is for, why it exists — while keeping the date/PR/SHA that makes it verifiable. Enforced where entries are written (`horus-consolidate`), not at any renderer: generating readable prose *from* shorthand is exactly what the OpenWiki/Graphify verdict ruled out, because generated prose cannot be trusted and drifts silently. Richer entries cost budget, so aged ones distil out to `.horus/archive/history.md` sooner via `horus distill-history` — "fuller entries" without that is just refilling the cap.

- **Prefer derived over generated for any documentation surface (2026-08-09, benchmark verdict).** A projection over committed files cannot hallucinate, refreshes free, carries provenance, and cannot drift; an LLM-authored artifact fails all four. `horus capabilities`, `horus backlog --tree --json` and the dashboard's live `.horus/` read are the pattern. A free MIT tool whose refresh is a model run is not cheap: cost scales with refresh frequency, so licence is the wrong axis to compare on.
