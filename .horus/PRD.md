...
next_action: "Continue the X3 away-mode kit before 2026-07-22: (3) unattended-escalation-channel — now scoped (PR cdd04b2 card update): Horus owns event wiring only; sink is hermes|webhook|none (default none), Hermes optional + best-effort. TWO owner design forks still open before it is dispatch-ready — hermes sink to DM vs a dedicated Horus topic, and which events escalate by default. Then (4) supervise-verify-merge-close (andon; settled live-probe definition is the precondition for unattended MERGE); (5) parallel-session-continuity-reconciliation; (6) cockpit-autonomous-dispatch-contract skill. Cut line unchanged: 0-3+5 give a trustable verify+escalate-only away loop."
next_prompt: "Fetch clean main. X3 (Autonomous dispatch facet) is primary; owner trip 2026-07-22, items 0-2 shipped, item 3 scoped around `hermes send`. Resolve the two open forks in unattended-escalation-channel.md with the owner (DM vs dedicated topic; default-escalation events), then implement it: sink abstraction (hermes|webhook|none), Hermes optional so a no-Hermes machine still works, best-effort so a failing sink never fails the run. `hermes send` shells out one line (no token, works even if the gateway is down). Branch -> PR; reproduce every gate, drive the real surface once."
