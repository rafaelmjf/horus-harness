...
- **accounts-isolation** — 6 shipped, 3 open.
...
- **An account-scoped question is answered from the session's measured identity, never from ambient config (#516).** A token that authenticates proves nothing about whose session holds it — the desktop app and the CLI sign in independently, so a valid token and a 200 reported the wrong account's usage confidently. Measure it (`session_identity()`), match accountUuid before organizationUuid (a team org holds several members with separate limit pools), and **refuse on "cannot tell"**: a wrong number is worse than none, because none cannot make someone act.
