...
**Backlog card parallel-safety gate** (2026-07-11, PR #148): `.horus/backlog/` cards gain optional `parallel: safe|exclusive` + `surface: <globs>` frontmatter (back-compat); new `horus/backlog.py` is the first code path that reads `.horus/backlog/` at all — `horus backlog list`/`claim <name> [--force]` warn-and-block a claim that overlaps an in-progress card's surface, or where either card is `exclusive`, or where overlap can't be verified (missing surface). Prerequisite gate before a multi-worker contention test on a shared backlog. Note: the claim→push→finish flow and any stale-claimed-card sweep remain a manual git/PRD convention, not (yet) enforced by `consolidate` — see Structure contract.
**Card-per-file backlog pilot** (2026-07-10): `.horus/backlog/` holds dispatch-ready cards; the first Codex claim→push→finish flow (manual `status:` edit) was frictionless; `rafaelmjf/horus-agent` is the first cross-project consumer.
