---
status: active
current_focus: "2026-08-01 — v0.0.80 shipped, closing the day's arc and the version skew with it: the installed CLI, the hosted app and the repo now all read 0.0.80, so the TUI finally shows the same backlog the repo does (Shaping 4 / Gated 2 / Deferred 1 / Shelved 64, vs 40/6/23 that morning). The release carried one defect class fixed at the root in four places — a display aggregating over states it does not distinguish — plus `shelved`, the PRD's stop-caching-command-output pass, and roadmap-branches v8. Backlog is deliberately 7 active."
next_action: "Do `session-close-ux-and-truthful-end-state` — the host-agnostic TERMINAL-vocabulary fix, which is the fourth and last instance of this release's defect class: the registry has no status meaning 'the owner closed it', so `stop_session` writes `failed` and 73 of 246 rows lie. Start at `registry.py:29`; membership is tested at :317, :457 and :498, so the new status must be terminal in all three or reaping loops. Then FIX THE BROKEN GATE: `codex-isolated-config-leak` is Gated on `account-login-verb`, which the sweep shelved, so it can never lift."
next_prompt: "Resume horus-harness; `git fetch --all --prune` first. Main is clean and v0.0.80 is released, published and deployed — installed CLI, hosted app and repo all agree, so `horus` is trustworthy again (it was 33 commits stale all of 2026-08-01). Backlog is 7 active / 64 shelved: read `horus backlog list` and `--shelved`, never the directory listing. The next build is the TERMINAL-vocabulary fix in `session-close-ux-and-truthful-end-state`. Note four active cards reference blockers the sweep shelved, and one of them (`codex-isolated-config-leak`) is Gated on one, so its gate is unreachable until that is repaired."
execution_recommendation: "continue-as-is — the owner intends the NEXT release (small, carrying just the TERMINAL fix) as an end-to-end Luna-max worker test, which is a separate authorization at that time; this fix itself is one bounded change to a status vocabulary."
