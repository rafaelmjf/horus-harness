...
next_action: "The kit is shipped but NOT yet exercised as ONE loop. Before relying on it during the trip, run a real end-to-end rehearsal: `horus envelope create` a bounded envelope, `horus schedule` a small real card dispatch on an isolated account, pair a `horus supervise` (verify+escalate-only first, then a second with `--allow-merge` + a `--probe`), and watch it escalate to Telegram and halt a dependent. Then close the umbrella `vision-branch-x3-scheduling-and-autonomous-execution` as converged (the Autonomous-dispatch facet is already promoted). Next backlog priority: vendor-neutral-delegation-tiers (high — neutral tier labels + the Claude<->GPT prior, wanted before the first real X3 dispatch picks a model)."
next_prompt: "Fetch clean main. The X3 away-mode kit (items 0-6) is fully shipped: the verify+escalate-or-merge away loop exists (envelope + schedule + `run --unattended` + `supervise` + `notify` + andon + the parallel-delivery signal + the cockpit skill). Before the 2026-07-22 trip, DOGFOOD the whole loop once end-to-end on a real small card (isolated account, scheduled dispatch + paired supervise, watch the escalation reach @horus_agent_rmjf_bot and a dependent halt). Then judge/close the vision-branch-x3 umbrella as converged, and pick up vendor-neutral-delegation-tiers (high). Work inline; reproduce every gate; drive the real surface once."
...
Seventeen active cards. **Autonomous dispatch — X3 away-mode kit COMPLETE (items 0–6 all shipped 2026-07-17)**; the only remaining X3 item is the umbrella `vision-branch-x3-scheduling-and-autonomous-execution`, now fully delivered and awaiting the owner's formal close-as-converged (the Autonomous-dispatch facet is already promoted). Before the trip, the kit still wants ONE real end-to-end rehearsal (see `next_action`). Delegation calibration: high — vendor-neutral-delegation-tiers (neutral low/medium/high/frontier labels + owner Claude<->GPT equivalence prior, wanted before the first real X3 dispatch picks a model); low explore — openrouter-provider-support (parked until tiers + kit). The rest: medium — horus-statusline-default (ship the status line as a portable `horus statusline` so a new machine needs no hand-config; overlaps account-settings-sync, and only one of them writes account settings), tui-branch-tree-glance, fleet artifact refresh, stale-datum overlap reconciliation, remote open-model probe, project workflow overrides, scoped machine requirements, and explore-converge-lifecycle (usage-ripeness flag only); low/deferred — account-settings-sync, Codex usage-window semantics, completion-receipt trimmings, init-CI (retire candidate per the 2026-07-17 branch tree), heartbeat stall detection, and product-naming (rename pinned to first external distribution; want a creative name).
