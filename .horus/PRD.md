---
status: active
current_focus: "2026-08-07 — Graphify 0.9.35 code-only trial pushed on `experiment/graphify` (`c22c79d`): 6,868 nodes/18,288 edges from 199 code files, zero LLM tokens, vendored xterm excluded, and portable Claude/Codex skills committed without Graphify hooks. Exact `explain`/`affected` queries surfaced callers, tests, provenance and lines; natural-language queries expanded to 114–331 nodes and the unlabeled 239-community report was noisy. OpenWiki authenticated rendering passed, but the owner judged its visualizer too flashy."
next_action: "Try one fresh Codex or Claude session from `experiment/graphify` and judge whether exact-symbol graph navigation improves a real change enough to justify the 9 MB committed graph plus update cadence; do not merge the experiment on synthetic benchmark claims alone. Separately decide whether to retain the now-verified OpenWiki deployment and its unmerged experiment branch. Main's earlier account-login and release choices remain open."
next_prompt: "Resume `experiment/graphify`; fetch first and verify `c22c79d`. Graphify 0.9.35 is installed on this machine. The committed code-only graph excludes `horus/assets/vendor/`; cache and machine paths are ignored. Both agent skill projections and optional root guidance are present, but Graphify's generated hooks were removed because they used machine-absolute paths without Horus fail-open guards. Prefer `graphify explain`/`affected`/`path`; broad `query` and the zero-LLM report were noisy. Focused graph probes and 79 instruction/skill tests passed."
execution_recommendation: "continue-as-is — no delegation was requested; the useful next evidence is one bounded owner-attended agent trial."
