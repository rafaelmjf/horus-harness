---
status: active
current_focus: "Dashboard projects-section resilience fix shipped (2026-07-11, PR #155, branch fix/projects-section-resilient, merged clean as 6934ae8 — CI green: freshness + pytest 3.12/3.13). Live-reported bug: one registered project raising while rendering (concrete trigger: a registered path with no .horus/ dir) 500'd the ENTIRE projects section. Fixed with a per-project guard (_project_column_safe renders a compact error card instead of propagating) plus a graceful-degrade fix in routines.resume_prompt for the concrete trigger. This was an interrupt, not a continuation of either open thread below — both remain untouched."
