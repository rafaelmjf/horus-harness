---
status: active
current_focus: "X3 PROMOTED TO PRIMARY (2026-07-17): PR #289's vision branch (scheduling & autonomous execution, from a parallel scheduled session) chosen as the driving thread, with the owner away 2026-07-22 for 6 days — the away-mode kit is the deadline. Groundwork this session: pathfinder e2e convergence test (receipt research/2026-07-17-roadmap-branches-convergence-test.md — strong structural convergence with the held-out tree; the C-branch divergence traced to owner push-back that lived only in a receipt → owner verdicts that must bind future planning go in card Reviews); X3 mini market scan (research/2026-07-17-x3-autonomous-dispatch-scan.md — YELLOW: scheduling commoditized by native Desktop tasks/Routines/Devin, but independent deterministic supervision + own-machine multi-account routing + continuity closure are uncovered; Devin is the value proof); Vision gained the Autonomous dispatch facet (out-of-scope amended: distributed stays out) + the agent-first structure boundary (adapt kanban/epics for agents — pull dispatch, andon, vision branches, WIP-as-collision-control; never rebuild a ticket system); skill calibration shipped (pathfinder v3 interactive intent confirm, market-scan v4, roadmap-branches v2 out-of-scope re-test); #289 cards revised (ordered, branch: key, capacity-pull, andon) + new standing-dispatch-envelope card. Post-#290: second calibration pass from the same session shipped in PR #291 (roadmap-branches v3 prior-trees-as-input + disposition discipline + Reviews-bind-verdicts; scope-cards v2 vision-branch emission; pathfinder v4 reuse-collapses-gate; market-scan v5 branch-check variant), and PR #292 added vendor-neutral-delegation-tiers (high, before first X3 dispatches), openrouter-provider-support (low explore), tui-branch-tree-glance (medium), plus the X3 dispatch-motivation Review (away/capacity/parallelism, NOT active-work cost saving; dividend datums ~6+/5- weakly concur; datum alias split sonnet-5/claude-sonnet-5 noted). horus-agent decision: it stays the overseer-ROLE repo (zero code) and becomes the first Branch-B pathfinder dogfood target to re-baseline its shaky PRD."
next_action: "Build the X3 away-mode kit in dependency order before 2026-07-22, each step a green committed-and-pushed PR: (0) standing-dispatch-envelope — the bounded pre-authorization rule + refusal guards everything unattended runs under; (1) unattended-dispatch-attachable-worktree-defaults; (2) schedule-local-dispatcher (one-shot first; capacity-pull trigger if time); (3) unattended-escalation-channel (adopt native transports, own only event wiring); (4) supervise-verify-merge-close (andon: escalation halts depends-on descendants; live-probe definition is a precondition); (5) parallel-session-continuity-reconciliation; (6) cockpit-autonomous-dispatch-contract skill. Honest cut line: 0-3+5 make a trustable away loop with verify+escalate-only; 4's unattended MERGE ships only if its live-probe design settles; 6 is cheap text. Dogfood each piece on a real card as it lands."
next_prompt: "Fetch clean main. X3 (Autonomous dispatch facet) is primary; owner trip starts 2026-07-22. Read the vision-branch-x3 umbrella card and the standing-dispatch-envelope card, then implement the away-mode kit in the order pinned in next_action, starting with standing-dispatch-envelope (rule text in PRD Rules + refusal checks at the horus run/schedule boundary). Branch -> PR per card; reproduce every gate yourself."
execution_recommendation: "plan-execution — seven bounded cards over 5 days, mostly parallel:unsafe on horus/cli.py so PRs land sequentially; a phased .horus/execution.md with dispatched workers fits the mechanical middle (cards 1-3) while the supervisor designs envelope + supervise; every dispatched phase doubles as X3 dogfood."
...
Twenty-three active cards. **Autonomous dispatch — X3 primary, away-mode kit due before the 2026-07-22 trip** (from PR #289, promoted to facet 2026-07-17; market check `research/2026-07-17-x3-autonomous-dispatch-scan.md`): high — standing-dispatch-envelope (item 0, the pre-authorization rule everything unattended runs under); medium — unattended-dispatch-attachable-worktree-defaults, schedule-local-dispatcher, unattended-escalation-channel, supervise-verify-merge-close, cockpit-autonomous-dispatch-contract, umbrella vision-branch-x3-scheduling-and-autonomous-execution; plus **parallel-session-continuity-reconciliation (high — joins the kit: scheduled dispatch makes sibling deliveries routine; two incidents in 24h, PRs #287/#289)**. Delegation calibration: high — vendor-neutral-delegation-tiers (neutral low/medium/high/frontier labels + owner Claude<->GPT equivalence prior, lands before first X3 dispatches); low explore — openrouter-provider-support (parked until tiers + kit). The rest: medium — tui-branch-tree-glance (backlog --tree projection + phone-first TUI render + receipts shelf), fleet artifact refresh, stale-datum overlap reconciliation, remote open-model probe, project workflow overrides, scoped machine requirements, and explore-converge-lifecycle (usage-ripeness flag only); low/deferred — account-settings-sync, Codex usage-window semantics, completion-receipt trimmings, init-CI (retire candidate per the 2026-07-17 branch tree), heartbeat stall detection, and product-naming (rename pinned to first external distribution; want a creative name).
