...
- `755347a` fix: make `horus tui` usable inside tmux — switch the client, don't refuse (#439)
  * fix: make `horus tui` usable inside tmux — switch the client, don't refuse
  Running Horus itself in a tmux pane was strictly worse than running it in a plain
  terminal, in three places that all assumed "Horus is outside tmux":
  - `default_target()` returned `current`, so a launch took over the Horus pane and
    the registry row became `original terminal only` — not attachable, unreachable
    from the phone.
  - `launch_tmux()` refused outright when `$TMUX` was set.
  - `attach_session()` refused, so the Sessions screen's attach action was dead for
    every session. That one was a plain bug.
  No nesting is actually needed: Horus and the sessions it creates share ONE tmux
  server (the `tmux` CLI resolves its socket from `$TMUX`), so the client is *moved*
  with `switch-client` instead of nesting a second one — no doubled prefix keys.
  `Ctrl-b L` toggles back, and the Horus pane is untouched meanwhile.
  Outcome messages are keyed on the host that actually ran (`target_ref`), never on
  ambient `$TMUX`: an owner inside tmux who forced `HORUS_TERMINAL_TARGET=current`
  did NOT get a switchable session, and "started in tmux" would send them chasing
  one that isn't there.
  Verified: full suite green (2344 passed) plus a live end-to-end probe on a private
  `-S` socket — real Horus code in a real pane created `horus-…` (attachable), moved
  its client onto it, and `switch-client -l` returned to the Horus pane. Reaping is
  untouched; the positive-confirmation invariant is unchanged.
  Cards: adds `tui-nested-tmux-navigation` (this change) plus `herdr-host-probe`,
  `session-host-protocol`, and `session-agent-state-awareness` for the pluggable
  session-host evaluation this came out of.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * test: give the live tmux client an explicit TERM, and report what tmux said
  CI failed the new live test on both Python versions: the probe client never
  attached. The cause was invisible because the test discarded the client's output —
  the exact failure mode the Rules name. A bare runner has no TERM, so
  `tmux attach` exits 1 with "terminal does not support clear".
  Two fixes, both matching existing practice: pass `TERM` (or `xterm-256color`) as
  pty_host and launch_window already do, and surface the client's own message on
  failure instead of asserting a bare empty string. Reproduced the CI condition
  locally with `env -u TERM` — failing before, passing after.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: continuity for the multi-session-management session (#439 + four cards)
  Records what this session decided, so it is not re-derived: herdr would replace
  Horus's tmux layer rather than Horus, and putting a 105-day-old single-maintainer
  binary under the phone path and hosted dashboard (both pinned to tmux `attach`)
  buys features mostly reachable from tmux — no switch. Three of its four
  attractive features were already met or bettered; only agent-state awareness was
  a real gap, and that is carded as additive.
  PRD: Shipped line for #439, refreshed handoff frontmatter, and readiness counts
  corrected for the four new cards (Attended 3→5, Shaping 35→36, Gated 5→6).
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `d7d2e6f` closure: ship tui-nested-tmux-navigation — #439 merged as 755347a
  Card archived with merge provenance; readiness counts back in sync
  (Ready—Attended 5→4). Next up is `herdr-host-probe` (order 15).
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `3d594a0` spike: run the herdr host probe — all four questions answered on v0.7.5 (#440)
  Receipt lands in the card's Reviews with the exact commands and observed output.
  Verdict: a herdr host is viable on every surface, with one hard limit.
  - Q1 headless creation: YES, but the CLI does not autostart its server, so a
    herdr host owes an `ensure_server` step tmux gives for free.
  - Q2 kill: YES (`pane close` kills the process). Liveness: PARTIAL —
    `process-info` gives pid/argv (better than tmux for confirming a runner), but
    there is no attached flag and no activity clock anywhere, confirmed against the
    248 KB bundled schema. Two of the four reaping conditions are unanswerable, so
    the host declares `liveness=False` and never reaps.
  - Q3 bare-PTY viewer: YES, streamable like the tmux viewer — but it frames the
    whole session UI, not one pane.
  - Q4 state: `idle`/`working`/`blocked`/`done`/`unknown`, plus `agent wait --until`.
    Detection is a versioned per-agent manifest fetched from the network, matching
    Claude's literal UI strings by screen region.
  That last point changes `session-agent-state-awareness`: a screen-scraper is a
  treadmill tracking another product's wording, not a one-off — prefer lifecycle
  hooks and host-supplied state. Also folded into `session-host-protocol`: no
  argv-exec/exit-code from the host, and isolated config dirs collide with
  `sun_path` limits since herdr derives its socket from its config dir.
  `session-host-protocol` ungated (Gated 6→5, Shaping 36→37). Probe hygiene:
  prebuilt release binary, private XDG_CONFIG_HOME/HOME, server stopped, nothing
  under the real $HOME, real tmux server untouched.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `7c4fc29` closure: ship herdr-host-probe — #440 merged as 3d594a0
  Card archived with merge provenance; Shipped line added and readiness counts
  back in sync (Ready—Attended 4→3). session-host-protocol is ungated and sitting
  in Shaping with its two remaining shape decisions named.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `d82232d` refactor: extract the session-host protocol (current · tmux · +herdr next) (#441)
  * refactor: extract the session-host protocol (current · tmux · +herdr next)
  Stage A of session-host-protocol. No behavior change: tmux and current-TTY
  launches, attachability labels, reaping, `--target` semantics and the phone path
  all work exactly as before, proven by the full suite plus a re-run of the
  nested-tmux live probe (real code in a real pane: created `horus-…`, labelled
  attachable, moved its client, `switch-client -l` returned).
  What moved. `horus/hosts/` now holds the protocol (`base.py`), the two hosts
  (`tmux.py`, `current.py`), and the host-neutral runner spec (`runnerspec.py` —
  the seam that makes a second host cheap, since any host that can run one command
  in a persistent pane inherits the 0600 spec, the PID handoff, the registry
  transitions, and the `horus run` worker path). `terminal_sessions` is now the
  façade: same public API, delegating to the resolved host.
  Capabilities replace string matching. Six declared flags — persistent, attach,
  viewer, liveness, reports_exit_code, state — and every leak where a caller
  compared a launch target to "tmux" is gone:
  - is_attachable/access_label ask the host, so a record naming a host this install
    lacks degrades to `original terminal only` instead of offering a dead attach.
  - run_executor asks "is there a durable host?" not `!= "tmux"`.
  - pty_host gates the managed web launch on `capabilities.viewer` and gets its
    argv from `viewer_argv()` instead of building `tmux attach-session` itself.
  - launch_window does the same for native windows.
  - argparse `--target` choices come from the host registry, so a new host is
    offered automatically; `current`/`tmux` keep working forever by rule.
  - the TUI, the plain terminal app and `horus open` call `launch_on(target, …)`
    rather than branching, so registering a host makes it launchable everywhere
    with no caller change.
  `liveness` is the load-bearing one: reaping needs four conditions and two of them
  (not attached, idle past a grace window) can only come from the host, so a host
  that cannot answer them has no reapable sessions at all. The
  positive-confirmation invariant is now structural rather than remembered — and
  the herdr probe already showed herdr is exactly such a host.
  Also adds `ensure_ready()` (a no-op for both current hosts) because herdr's
  server does not autostart, and `[terminal] host` in config.toml for per-machine
  selection alongside the existing HORUS_TERMINAL_TARGET override.
  Tests that patched implementation internals now patch the host that owns them;
  six new tests cover the seam itself — protocol conformance, the `current` floor,
  selection precedence, unknown-host degradation, `launch_on` refusals, and that a
  host without `liveness` is never even asked for reap candidates.
  Not in this PR, deliberately: renaming `tmux_runner` to `pane_runner`. The module
  name is embedded in the specs of sessions running right now, so it needs a shim
  and is pure cosmetics — tracked on the card.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: continuity for Stage A of the session-host protocol
  PRD Shipped line for #441 plus the handoff refreshed to name Stage B (the herdr
  host) as the next step. The card keeps both shape decisions it was holding, now
  settled in its Reviews: the host is never asked for the agent's outcome (the
  registry is the single source of truth, already true on every host), and the
  herdr host is a separate PR so Stage A stays unambiguously no-behavior-change.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `837a522` feat: the herdr session host — three hosts, one protocol (#442)
  * feat: the herdr session host — three hosts, one protocol
  Stage B of session-host-protocol, built on what the probe measured against herdr
  v0.7.5 rather than on its docs. The whole host is ~370 lines and needed no change
  to any caller: `hosts/__init__.py` registers it, and the TUI, the plain terminal
  app, `horus open` and `--target herdr` pick it up through `launch_on()`.
  Verified end-to-end against a real herdr server (isolated config dir, torn down):
  a `launch_on("herdr", …)` created workspace pane `w1:p1`, the registry row reads
  `launch_target=herdr` / attachable, **the runner started and reported its PID**
  (so the host-neutral runner spec carried the entire delivery path unchanged),
  `viewer_argv` returned `["herdr"]` after focusing the workspace, `reap_orphans`
  returned nothing even with the record terminal and a zero grace window, and
  `stop_session` killed the runner leaving no panes behind. Plus a live test in the
  suite (skipped when herdr is absent) covering server start → pane → run →
  process-info → close-kills-the-process.
  The three declared differences from tmux, each measured:
  - `ensure_ready()` starts `herdr server`; the CLI never autostarts it, and every
    command against an absent server fails with a bare ENOENT on the socket.
  - `liveness=False` — no attached flag, no activity clock anywhere in its API, so
    two of the four reaping conditions are unanswerable and its panes are never
    reaped. A test asserts reaping does not even query herdr.
  - `reports_exit_code=False` — `pane run` types into the pane's shell, so the
    runner is a shell child. Costs nothing: the registry was already the single
    source of outcome truth on every host.
  And the capability tmux lacks: `state=True`, with `agent_state()` returning
  idle/working/blocked/done/unknown. Verified live by pushing states through
  `herdr pane report-agent` and reading each one back — which is also the path
  Claude hooks could drive for `session-agent-state-awareness`.
  Two things the live runs corrected in this implementation:
  - `pane focus` is DIRECTIONAL (left/right/up/down), not "focus this pane". Bringing
    a session into view is `workspace focus <workspace_id>`, with the workspace read
    from `pane get` rather than parsed off the ref.
  - agent state comes from `pane get`.`agent_status`, not `agent explain`: the latter
    errors `agent_not_found` on a pane with no detected agent, which is a normal
    state and not a failure.
  herdr is opt-in via `[terminal] host` / HORUS_TERMINAL_TARGET, and sits behind tmux
  in the `auto` order: tmux is the proven host, it can be reaped, and it backs the
  phone path, so an install must never silently switch hosts because a binary
  appeared on PATH.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: continuity for the herdr session host (#442)
  Shipped line for the host plus the handoff refreshed to the owner-chosen next
  step: `horus tui <host>` as a positional cockpit front door, a global
  where-to-host default in the TUI Defaults screen, and the `resolve()` fix so a
  TUI running inside a host launches agents onto that same host.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `67bb8e2` feat: `horus tui <host>` cockpit front door + a persisted session-host default (#443)
  * feat: `horus tui <host>` cockpit front door + a persisted session-host default
  Three things, all composition over the host protocol rather than new mechanism.
  **`horus tui tmux` / `horus tui herdr`** (owner chose the positional over `--in`)
  opens the cockpit inside a host, which is what makes the nested workflow
  practical: the TUI lives in its own pane and each launch becomes a sibling session
  you switch to and back from. Two behaviours are the design — it is idempotent (one
  cockpit per host, found by ref; re-running re-attaches rather than accumulating
  cockpits you then have to tell apart), and it never nests (already inside that
  host ⇒ run in place). Bare `horus tui` and `horus tui current` are unchanged.
  Choices come from the host registry, so a new host is offered without touching
  argparse, and the cockpit command prefers the console script on PATH but falls
  back to this interpreter's module entry point, so a checkout opens a cockpit
  running the checkout.
  **A persisted where-to-host default**, surfaced in the TUI Defaults screen (`d`)
  beside posture and window, backed by a new `config.set_terminal_host()`.
  `[terminal] host` has had a reader since #441 but no writer and no UI, and is now
  a managed table (so it round-trips and is never duplicated by an unrelated write).
  Unavailable hosts stay listed and are marked "(not installed here)" rather than
  hidden — an owner setting up a machine needs to see herdr is an option they have
  not installed. Picking `auto` reports what it resolved to, since otherwise the
  owner cannot tell what they just chose.
  **A resolution fix that matters more than the command.** `hosts.resolve()` never
  considered which host Horus was already running inside, so a cockpit in a herdr
  pane launched agents onto tmux — first in AUTO_ORDER and available — leaving the
  cockpit and its agents on different servers with no way to switch back. It now
  prefers the enclosing host, ranked below the env override and an explicit config
  choice, both of which are the owner saying otherwise on purpose.
  Verified live, not just unit-tested: `horus tui tmux` under a real pty created one
  cockpit, ran the actual TUI in it (captured "HORUS · Projects" from the pane),
  attached the caller's own terminal to it, and a second invocation re-attached
  rather than creating a second cockpit. Full suite 2380 passed.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: continuity for the cockpit front door (#443)
  Shipped line for #443 plus the handoff refreshed: the three-host layer is
  complete, herdr 0.7.5 is now genuinely installed on this machine (so the live
  test runs rather than skipping), and the one unverified thing left is a real
  Claude session on the herdr host — specifically whether manifest detection
  reports `blocked` on a genuine permission prompt.
  The card records that only the `tmux_runner` → `pane_runner` rename remains, and
  flags that its tier/parallel stamps now describe shipped work rather than a
  rename — a rescope for a refine pass, not a call to make mid-merge.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `d9ffa7b` fix(test): pin every host's availability in selection tests (#444)
  * fix(test): pin every host's availability in selection tests
  Installing herdr on this machine broke two tests that had passed on every branch:
  they patched only tmux's availability and asserted the fallback landed on
  `current`, which was true when tmux and current were the only hosts. With herdr
  registered AND installed, `auto` correctly resolves to herdr instead.
  The tests were asserting about the machine, not the code — and CI could never
  catch it, because herdr is not installed on the runner. So the failure only
  appears once someone actually installs a host, which is precisely when they are
  least expecting a test to break.
  `_only_available(monkeypatch, *host_ids)` pins availability for every registered
  host and clears the enclosing-host env signals (being inside a host outranks
  AUTO_ORDER). It asserts the known host set, so registering a fourth host fails
  here with an explanatory message rather than silently making these tests
  environment-dependent again. Adds the case the gap implied: herdr as the only
  persistent host resolves to herdr.
  Verified both ways — full suite green with herdr installed (2382), and green with
  it absent from PATH (the live herdr test skipping as designed).
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: record the machine-state test rule from #444
  Adds a PRD rule for the class of bug, not just the fix: a test that reads machine
  state must pin all of it, because CI cannot catch the failure (the runner lacks
  the optional dependency) and it therefore surfaces when someone installs a tool.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `ebd6bff` fix: four host-selection leaks found in a pre-release sweep (#446)
  * fix: four host-selection leaks found in a pre-release sweep
  All four are mine, from Stage A: I widened `--target` and the capability checks
  but three call sites still reached for tmux directly. Each is only reachable once
  someone opts into a non-default host — exactly what the herdr work enables.
  1. `launch_window` never called `ensure_ready()`, so a herdr window launch
     (`horus open`, dashboard) died on a bare ENOENT from the socket instead of
     starting the server.
  2. **The worst, and a release blocker.** `pty_host` gated the managed browser
     terminal on the RESOLVED host's `viewer` capability, then launched through the
     tmux façade, then asked the resolved host for a viewer onto a tmux ref. On a
     herdr-configured machine that returns None and the in-app terminal dies. It did
     not merely pick the wrong host, it mixed two. Gate and launch now share one
     resolved host.
  3. `launch_detached_run` hardcoded tmux while argparse offered herdr, and the
     `--detach` guards compared against the literal "tmux" — so `--target herdr` was
     offered and then refused, and a herdr machine could host no worker at all. The
     guards now ask whether a host is *persistent*, via `persistent_hosts()` /
     `default_persistent_host()`, so `--unattended` follows the configured host
     rather than assuming tmux.
  4. Diagnosability: every herdr verb fails identically when its server is down (a
     bare ENOENT), so `attach()` reported "herdr does not know pane w1:p1" and sent
     the reader hunting a pane problem that did not exist. Failure paths now check
     the cheap structural cause first and say "the herdr server is not running".
  Each has a regression test that fails on the old code; #4 verified live against a
  genuinely stopped server. Suite green both ways — 2387 with herdr installed, 2385
  +1 skipped without it.
  Worth recording rather than hiding: CI could not have caught any of these, for the
  same reason as #444 — the runner has no herdr, so herdr paths are never exercised
  there. That is a standing gap in release confidence, not something this fixes.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * fix: revive a dead cockpit, and bind it to the running Horus
  Two bugs an owner's real trial found that my pty-driven runs walked straight past.
  `uv run horus tui herdr` opened the right space and tab with a BLANK terminal.
  **1. Attached to a dead cockpit.** herdr persists workspace *structure* across a
  server restart but not processes, so the `horus-cockpit` workspace comes back with
  a bare `/bin/bash` in it. `_find_cockpit` matched on the LABEL, concluded a cockpit
  existed, skipped creation, and attached to a corpse. A ref existing is not evidence
  that anything is running in it — the same reason the reaper never trusts a ref's
  mere presence, and the `liveness=False` theme once more: herdr cannot report what
  is alive, so Horus must look at the process.
  Now `_cockpit_is_live()` checks for our own process in the pane and
  `_revive_cockpit()` re-runs the TUI in the pane we already have, so it self-heals
  without leaving a second cockpit to disambiguate. With several stale labelled
  workspaces it prefers a live one, and never closes the others: one may be a live
  cockpit and guessing is how a live session gets killed.
  Reproduced and verified live: restored ref reports TUI alive False (the blank
  pane), revive brings it back, the pane paints "HORUS · Projects  4 live", and the
  workspace count stays at 1.
  **2. The cockpit could run a different Horus than the caller.** `_tui_command()`
  used `shutil.which("horus")`, so whichever `horus` sat first on PATH won —
  observed live running the global 0.0.77, a version that does not even have this
  command. Now bound to `sys.executable -m horus tui`, making "the cockpit runs what
  I ran" true by construction rather than by PATH order. Verified live: the checkout's
  venv.
  My earlier "idempotent, verified 3 times" claim was only ever true within one
  server lifetime; the restart case is what broke it, and the test now covers it.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: continuity for the pre-release host-selection sweep (#446)
  Records the seven bugs and, more importantly, the standing lesson: CI cannot
  exercise the herdr paths, and the owner's first real trial found two bugs that
  every programmatic check had walked past. That belongs in the release framing,
  not buried.
  Also parks the open design question the trial raised — how herdr sessions should
  be placed — with the owner's constraint (rarely parallel per project) and the
  cheap independent win (opaque `horus-<id12>` labels should carry the project name).
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `df66d1d` feat: herdr layout — Horus space + Agents space, one tab per project (#447)
  * feat: herdr layout — Horus space + Agents space, one tab per project
  Owner decision after a hands-on trial: porting tmux's session-per-agent 1:1 gave a
  flat list of spaces that read wrong. herdr's own idiom is agents as tabs inside a
  space with the sidebar reporting state across them, so:
  - **`Horus`** space holds the TUI (the cockpit).
  - **`Agents`** space holds one tab per session, labelled with just the project
    name. herdr reports agent_status per *tab*, so the tab strip doubles as a status
    bar — which is why no state is encoded in the label.
  Project *grouping* was considered and dropped: this owner rarely runs parallel
  sessions per project, so a space per project would usually hold exactly one tab.
  Three placement behaviours, each verified against a real herdr rather than assumed:
  - The first session **reuses the fresh space's root tab** (a new space arrives with
    a tab labelled "1"; creating another would leave that dead beside it).
  - A finished session's tab **closes with it** — no accumulation from normal use.
  - A tab holding **only a shell is reused**. herdr restores workspace structure
    across a server restart but not processes, so after a restart the tab is there
    and the agent is not; without reuse every restart would silently add a duplicate.
    Live: relaunching a project returned the existing pane and the tab list did not
    grow. A *busy* same-project tab gets a sibling instead, never trampled.
  Liveness is judged by "is the foreground process a shell", not by matching our own
  command line — argv matching breaks the moment the runner's command changes, and it
  already bit the cockpit check once. Consequence: an unrelated command in a Horus
  tab reads as busy and is left alone, which is the right side to err on.
  The tmux cockpit keeps its `horus-cockpit` session name (that prefix is how
  Horus-owned tmux sessions are identifiable, and what `_live_tmux_sessions` filters
  on) while herdr's is the human-facing `Horus`. An asymmetry, deliberately.
  Suite 2397 passed. Nine new tests cover the labels, per-project tabs, idle reuse,
  the busy-sibling rule, and that liveness keys on the process name.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  * closure: continuity for the herdr space layout (#447)
  Shipped line for the two-space layout and the three placement behaviours, plus the
  handoff pointing at the owner's hands-on trial as the next step — the tab strip and
  per-tab state indicator are the two things no check here can see, and real trials
  have caught bugs twice that API-level checks missed.
  Also states the release framing to carry forward: herdr is less proven than tmux,
  because CI cannot exercise its paths and every herdr bug this session was found by
  hand.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
  ---------
- `aa177a3` Bump version to 0.0.78 (#448)
  Carries two sessions of unreleased work: `horus sync`, cockpit remote-freshness +
  inbound Sync, the consolidate cap signal and launchable pinned/older models from
  the earlier session, plus the whole three-host session layer from this one.
  Also records the owner's versioning intent as a rule, so a future session does not
  guess: `0.1` is reserved for the first version considered stable enough to hand to
  someone else to test. Until then releases stay on 0.0.x however structural the
  change — the session-host layer ships in 0.0.78, not 0.1. A patch bump here does
  not mean "small".
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `c38cfc6` fix: user-facing strings that assumed tmux was the only host (#449)
  Queued for 0.0.79. Both shipped in 0.0.78, from the same omission: the Stage A
  widening taught the *guards* about capabilities but left the *text* naming tmux.
  **The one that actually misleads.** Every launch and attach message said "Ctrl-b L
  toggles between it and Horus" and "started in tmux". On herdr that key does
  nothing — `previous_workspace` is unbound in its default config — so the hint sent
  the owner at a no-op. `switch_hint` now belongs to the host beside its
  capabilities: tmux says `Ctrl-b L`, herdr points at `Ctrl+b w` and names the
  `Horus` space, and `current` has none because it never switches in place. The
  launch message names the host that ran rather than saying "tmux".
  **Five stale help strings.** `--detach` on both `open` and `run` promised
  `--target tmux` although the guards now accept any persistent host, and `attach`
  and `stop` described themselves as tmux-only. All now say "a persistent host".
  Deliberately NOT changed: the three `reap` strings still say tmux, because reaping
  really is tmux-only — it is the only host that can report attached/idle, which is
  two of the four conditions Horus requires before killing anything. A test pins that
  exemption so a future sweep does not "fix" a true statement.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `5a76c4a` closure: v0.0.78 released, deployed and verified; 0.0.79 queue opened
  Records the release and its verification (PyPI JSON + simple index, /health 0.0.78,
  / still 403, global CLI upgraded), and that herdr shipped labelled experimental
  because CI exercises none of its paths.
  Opens the 0.0.79 queue with #449: strings that assumed tmux was the only host. The
  launch/attach hint was the misleading one — "Ctrl-b L" does nothing on herdr, whose
  default config leaves previous_workspace unbound — so switch_hint now belongs to
  the host. The reap strings stay tmux-only because that is true, pinned by a test.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `573494f` closure: card the CI gap, and correct a misread trigger (#450)
  Two corrections found while checking whether the standing items were actually
  carded.
  **A claim I made twice was wrong.** I said cutting v0.0.78 un-gated
  `verify-guidance-long-running-services` and `audit-advisory-interval`. It did not:
  their reasons say "TRIGGER: released to Ready—eligible when the drill is armed and
  this leg is either used or dropped", where "released" means *becomes selectable*,
  not *a version ships*. Both are still deferred. The false claim had reached PRD
  frontmatter, so it is corrected there, and a Rule now records the wording trap so
  the next reader does not repeat it.
  **The session's standing lesson had no card.** The herdr paths are structurally
  unreachable by CI — no herdr on the runner — and that produced ten defects in one
  day that a green suite never saw: seven from a by-hand sweep, two from the owner's
  first trial, one from their second. It was recorded only inside two other cards'
  prose, so there was nothing to act on. `optional-host-ci-coverage` now holds it,
  with the actual trade-off (install a third-party binary in CI, accept
  demonstration-only confidence and say so, or replay recorded herdr responses as
  dated fixtures) as an owner decision rather than a pre-chosen task.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `9cbfe91` docs: recover the real-Claude-on-herdr evidence from an unmerged PR (#451)
  Found while verifying the standing items were carded: PR #445 was never merged, so
  the only record of the real Claude trial on the herdr host — including the
  false-idle finding that redirected `session-agent-state-awareness` toward hooks —
  was not on main, while later continuity already cited it.
  The branch had gone stale by eight merges, so merging it would have reverted the
  host-selection fixes, the layout work and the release. Its commit was pure
  additions to one card (51 lines, no deletions) and nothing on main had touched that
  card since the branch point, so the content is recovered directly and #445 is
  closed as superseded rather than merged.
  The evidence itself: herdr identifies Claude as an agent and reports
  `idle → working → blocked`, with `blocked` firing correctly on a real Write
  permission prompt. But Claude's trust-folder dialog — a blocking prompt — is
  reported as `idle`, because herdr's manifest matches the literal "enter to select"
  and that dialog says "Enter to confirm". A blocking prompt read as idle is the
  failure that destroys the feature's value, and it is the structural cost of
  screen-scraping rather than a herdr defect.
  Also adds the Rule the loose end earned: a green PR is not a landed PR, and
  `horus close --check` already lists unmerged remote branches for exactly this.
  Claude-Session: https://claude.ai/code/session_01C9XXWEStFnrTtY3w2yCq5g
- `aa925a3` feat: make TUI selections clickable (#453)
