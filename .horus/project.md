...
status: active
current_focus: "v0.0.9 live on PyPI; riding the next release: projection-sync badge (PR #68) and the user-requested VS Code launch destination tier 1 (PR #70 — 'Open in' select, code-CLI opener, doctor check; 583 tests green). Remaining UX-hardening items are per-OS/lifecycle direct work (graceful hooks, onboard artifact commits, startup visibility, install-smoke CI). Horus remains the durable memory plane (no session hosting)."
