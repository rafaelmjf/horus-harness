...
- **Hook guard invariant: hooks signal via stdout JSON + exit 0; commands carry a per-OS silence guard** — a missing/broken CLI must be a silent no-op on every machine a repo reaches. POSIX `… || exit 0` on every `command` (sh + Git Bash — the shells Claude runs hooks through; stderr stays in the debug log at exit 0); Codex `commandWindows` uses a PS 5.1-safe `Get-Command` probe (Windows Codex hooks run in PowerShell, `||` is PS7-only). Because hooks never use exit codes to signal, the guard can't mask a decision — never add an exit-code-2 hook without revisiting this. `doctor machine` is the visible signal. ↳ history.md
- **Projected artifacts are committed continuity, not gitignored** — `.claude/settings.json`, `.claude/skills`, `.agents/skills`, `.codex/hooks.json` (specific paths; `settings.local.json` never staged) ride the continuity pathspec: init/onboard installs the full projection set and commits it in one pass, `close --commit` covers later refreshes. The guard invariant above is what makes committing hooks safe on horus-less machines. ↳ history.md
- **Every release gets an install smoke on all three OSes** — fresh ubuntu/windows/macos runners install from PyPI and probe `horus --version` + dashboard `/health` on each published release ("reproduce the gate" for releases); cut releases promptly after meaningful merges — merged-but-unreleased work caused live confusion twice on 2026-07-02.
- **`doctor machine` checks every command hook, and only its fails flip the exit code** — a broken third-party hook spams every tool call just like a Horus one, so no marker filtering; warns (Tk, gh) are advisory. NB the older `project`/`instructions` sections still flip rc on warn — unify deliberately before adding another doctor section.
...
- **Every dashboard form POST is PRG** — redirect with query-param banners (`_notice`); never answer a POST with page/fragment HTML at the action URL (unstyled page, refresh re-submits the action). Third endpoint caught doing it (ignore → unignore → onboard) made this the default contract for new endpoints. ↳ history.md
...
- **A stale-build server never writes artifacts** — when the on-disk install is newer than the dashboard's loaded build (`selfupdate.build_state`, disk-newer only so dev checkouts never warn), every page banners "restart Horus" and the artifact-mutating endpoints refuse. Guard chosen over shelling out per endpoint: uniform, no PATH fragility, and a restart is required anyway before badges are trustworthy. ↳ history.md
- **Self-update must land or fail loudly** — `run_upgrade` verifies the on-disk dist actually reached the latest release (a pinned env's "success at the old version" is reported as failure with the one-time `--python` migration command), migrates interpreter-pinned envs via `uv tool install --force --python <floor>`, and uses `--reinstall` (implies `--refresh`; `uv tool upgrade` rejects a bare `--refresh`, uv 0.11). ↳ history.md
...
- **A launch *destination* is not a session** — `target=vscode` only opens/focuses the folder: no agent spawned, nothing registered, and a resume prompt is never injected into a surface Horus doesn't control (its copyable text stays on the project page). Auto-starting the agent there (`.vscode/tasks.json`) needs its own design pass first (trust gate, `.vscode/` ownership, prompt-in-committed-file). (User request, 2026-07-02.)
...
- **`.vscode/` is a user surface, not a Horus one** — Horus creates `tasks.json` only when absent, never merges/overwrites (jsonc; the snippet is printed for manual adds), and offboard removes only a byte-identical file. Anything Horus writes there must be static and secret-free: continuity/prompts resolve at run time (`$(horus resume)`), accounts stay ambient — never bake an account env or a resume prompt into a committable file. Auto-run (`folderOpen`) rejected for the cheap tier (trust gate + would force a machine-local spec). (User-chosen, 2026-07-02.)
- **Projections move together across agent targets; drift is user-visible** — a major change to skills/hooks/managed blocks must reach every agent surface (Claude + Codex today, more later), never one; the dashboard indicates per-project projection-sync state rather than relying on the user to remember. (User-set, 2026-07-02.)
- **Projection sync compares each surface to the installed CLI, never surfaces to each other** — per-target `upgrade_project` dry-runs are the single staleness truth (`horus/projection_sync.py`); verdicts: in sync / `<agent>` behind (n) / CLI outdated (ahead wins — remedy is upgrading the CLI). Read-only badge before any auto-sync. Hooks read as "behind" on any difference until they get a generation stamp (→ roadmap residual). (User-approved design, 2026-07-02.)
